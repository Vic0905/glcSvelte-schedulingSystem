<script>
  ...
  // Hoisted to module scope (instead of living inside formatScheduleCell) so
  // it can also be reused in the cellClick handler below.
  ...
  function formatScheduleCell(cell) {
    ...
    // customSchedule is a multi-relation array, so search it for a
    // break-type tag instead of assuming `first.customSchedule` is a
    // single object.
    ...
    return h(
      ...
      [
        ...
        h('div', { class: 'flex justify-start w-full mt-1 gap-1 flex-wrap' }, [
          ...
          // render a badge for every remaining tag (e.g. a "Sub Class" tag
          // alongside others), instead of only the single first tag
  ...
  /**
   * dailySchedule records carry a single `date` field (not a start/end range),
   * so every fetched record already belongs to exactly one day — no
   * range-vs-holiday filtering is needed anymore.
   */
  function normalizeSchedules(rawSchedules) {
    return rawSchedules.map((s) => ({
      ...
      date: s.date?.split(' ')[0],
      status: s.status,
      // Keep the full array instead of unwrapping to a single object, so
    ...
    if (gridInstance) {
      ...
      gridInstance.on('cellClick', (_e, cell) => {
        ...
        combineModal.open({
          ...
          // combineModal's dropdown is single-select and only manages the
          // break-type tag, so pull just that one out of the array instead
          // of passing the whole array (or a single object) to it.
...
<CopyModal bind:this={copyModal} sourceDate={selectedDate} onrefresh={refreshWithScroll} roomType="mtm" />
<ShowStatusModal bind:this={showStatusModal} sourceDate={selectedDate} roomType="mtm" onrefresh={refreshWithScroll} />
<ClearDayModal bind:this={clearDayModal} {selectedDate} onrefresh={refreshAfterClearDay} roomType="mtm" />
