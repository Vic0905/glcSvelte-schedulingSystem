<script>
  ...
  const stickyStyles = `
    #mondayTeacherGrid .gridjs-wrapper { max-height: 700px; overflow: auto; }
    #mondayTeacherGrid th { position: sticky; top: 0; z-index: 20; box-shadow: inset -1px 0 0 #ddd; }
    #mondayTeacherGrid th:nth-child(1), #mondayTeacherGrid td:nth-child(1) { position: sticky; left: 0; z-index: 15;  box-shadow: inset -1px 0 0 #ddd; }
    #mondayTeacherGrid th:nth-child(1) { z-index: 25; }
  `
  ...
  async function loadMondayTeacherSchedule() {
    ...
    try {
      ...
      if (teacherGrid) {
        const wrapper = document.querySelector('#mondayTeacherGrid .gridjs-wrapper')
        const scroll = { top: wrapper?.scrollTop || 0, left: wrapper?.scrollLeft || 0 }

        teacherGrid.updateConfig({ data }).forceRender()

        requestAnimationFrame(() => {
          const w = document.querySelector('#mondayTeacherGrid .gridjs-wrapper')
          if (w) {
        ...
        }).render(document.getElementById('mondayTeacherGrid'))
      }
    } catch (error) {
...
<div class="p-6 bg-base-100">
  <div class="flex items-center justify-between mb-4 text-2xl font-bold text-primary">
    <h2 class="text-center flex-1">Monday Teacher View Table</h2>
  ...
  <div id="mondayTeacherGrid" class="border rounded-lg"></div>
</div>
