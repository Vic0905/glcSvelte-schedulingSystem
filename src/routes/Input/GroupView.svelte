<script>
  ...
  function getWeekRangeDisplay(startDate) {
    ...
    const optStart = { month: 'long', day: 'numeric' }
    const optEnd = { month: 'long', day: 'numeric', year: 'numeric' }
  ...
  const createBadge = (text, color) => h('span', { class: `badge ${color} badge-xs whitespace-nowrap` }, text)
      ...
      if (groupGrid) {
        ...
        groupGrid = new Grid({
          ...
          className: {
            ...
            td: 'border p-1 text-center align-middle',
          },
          style: {
...
<div class="p-6 bg-base-100">
  ...
  <div class="mb-2 flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-2">
      <button class="btn btn-outline btn-sm" onclick={() => changeWeek(-1)} disabled={isLoading}>
        &larr; Prev Week
      </button>
      <button class="btn btn-outline btn-sm" onclick={() => changeWeek(1)} disabled={isLoading}>
        Next Week &rarr;
